...
class SystemFunction < ActiveRecord::Base
  ...
  def self.create_by_hash(attrs ={})
    attrs = {:is_active => true}.merge(attrs)
    group = SystemFunctionGroup.find_or_create_by_name(attrs[:group_name])
    sf = group.system_functions.create(:subject_title => attrs[:subject_title],:default_action => attrs[:default_action],:is_active => attrs[:is_active])
